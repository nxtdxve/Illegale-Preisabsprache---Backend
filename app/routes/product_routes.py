...
def add_product():
    ...
    if len(urls) < 2:
        return jsonify({"error": "Mindestens zwei URLs von verschiedenen Einzelhändlern müssen angegeben werden."}), 400
    ...
    if len(retailer_data) != len(set(domains)):
        return jsonify({"error": "Mindestens eine URL entspricht keinem registrierten Einzelhändler."}), 400
